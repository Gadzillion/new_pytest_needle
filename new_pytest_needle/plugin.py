...
def pytest_addoption(parser):
    ...
    group.addoption('--xbrowser', action='store', dest='xbrowser',
                    metavar='xbrowser', default=DEFAULT_BROWSER,
                    help='type of browser')

    group.addoption('--land', action='store', dest='land',
                    metavar='land', help='list of landings')
...
def needle(request):
    ...
    options = {
        ...
        'xbrowser': request.config.getoption('xbrowser'),
        'land': request.config.getoption('land')
    }

    return NeedleDriver(**options)
